...
class MainActivity : AppCompatActivity() {
    ...
    private var dataList :ArrayList<String> = arrayListOf("aaaaa","bbbbbbbbbbbbbbbbbbbbbbbb","cccccccccccccccccccccccccccc","dddddddddddddddddddddddddddd","eeeee","ffffffffff","ggggggg","hhh","iii","jjjjj","kkkkk")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.removeall.setOnClickListener {
            ...
            binding.constraintlayout.getLocationOnScreen(intArr)
        ...
        for(i :Int in 0 until dataList.size){
            ...
            button.post(Runnable {
                if(button.x != 0f && i !=0){
                    setConstraintMargin(constraintSet,buttonList,button,i)
                }
                button.setOnClickListener {
    ...
    fun setButtonConstraint(constraintSet: ConstraintSet, button: Button, type:Int){
        if(type==0) {
            ...
            when(type%3) {
                0-> {
                    setConstraintConnect(constraintSet, button.id, ConstraintSet.BOTTOM, buttonList[buttonList.size - 1].id, ConstraintSet.TOP)
                    setConstraintConnect(constraintSet, button.id, ConstraintSet.RIGHT, ConstraintSet.PARENT_ID, ConstraintSet.RIGHT)
                    setConstraintConnect(constraintSet, button.id, ConstraintSet.LEFT, ConstraintSet.PARENT_ID, ConstraintSet.LEFT)
                }
                1->{
                    setConstraintConnect(constraintSet,button.id, ConstraintSet.BOTTOM, buttonList[buttonList.size - 1].id, ConstraintSet.TOP)
                    setConstraintConnect(constraintSet,button.id, ConstraintSet.RIGHT, ConstraintSet.PARENT_ID, ConstraintSet.LEFT)
                    setConstraintConnect(constraintSet,button.id, ConstraintSet.LEFT, buttonList[buttonList.size - 1].id, ConstraintSet.RIGHT)
                }
                2->{
                    setConstraintConnect(constraintSet, button.id, ConstraintSet.BOTTOM, buttonList[buttonList.size - 1].id, ConstraintSet.TOP)
                    setConstraintConnect(constraintSet, button.id, ConstraintSet.RIGHT, ConstraintSet.PARENT_ID, ConstraintSet.RIGHT)
    ...
    private fun setConstraintConnect(constraintSet: ConstraintSet, startID :Int, startSide :Int, endID : Int, endSide : Int){
        constraintSet.connect(startID, startSide,endID, endSide, 0)
    }
}